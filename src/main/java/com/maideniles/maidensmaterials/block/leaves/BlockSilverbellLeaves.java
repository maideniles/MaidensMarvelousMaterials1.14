...
public class BlockSilverbellLeaves extends LeavesBlock {
    public BlockSilverbellLeaves(Properties properties) {
    ...
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!worldIn.isRemote()) {
            ...
            if (shears.getItem() == ModItems.PRUNING_SHEARS.get()|| player.isPotionActive(MaidensPotions.FLORAL_FORTUNE_EFFECT.get())) {
                ...
                ItemEntity blossom = new ItemEntity(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItems.SILVERBELL_BLOSSOMS.get(), 1));
