...
public class BlockWisteriaLeaves extends LeavesBlock {
    public BlockWisteriaLeaves(Properties properties) {
    ...
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!worldIn.isRemote()) {
            ...
            if (shears.getItem() == ModItems.PRUNING_SHEARS.get() || player.isPotionActive(MaidensPotions.FLORAL_FORTUNE_EFFECT.get())) {
                ...
                ItemEntity blossom = new ItemEntity(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItems.WISTERIA_BLOSSOMS.get(), 1));
