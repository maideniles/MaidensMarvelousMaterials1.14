...
public abstract class MyAbstractTreeFeature<T extends IFeatureConfig> extends Feature<T> {
    ...
    public MyAbstractTreeFeature(Function<Dynamic<?>, ? extends T> p_i49920_1_, boolean doBlockNofityOnPlace) {
    ...
    protected static boolean isDirtOrGrassBlockOrFarmland(IWorldGenerationBaseReader worldIn, BlockPos pos) {
        return worldIn.hasBlockState(pos, (p_214586_0_) -> {
            ...
            return Block.isDirt(block) || block == ModBlocks.ornamentalGrass.get() || block == Blocks.GRASS_BLOCK || block == Blocks.FARMLAND;
        });
    }
    ...
    public MyAbstractTreeFeature<T> setSapling(IPlantable sapling) {
